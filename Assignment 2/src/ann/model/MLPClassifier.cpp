    ...
    bool make_decision)
{
    ...
    double_tensor results = xt::zeros<double>({pLoader->get_total_batch(), pLoader->get_batch_size()});
    ...
    for (auto batch : *pLoader)
    {
        ...
        {
            results = Y;
            // cout << "First result = " << results << endl;
            // cout << "Y = " << Y << endl;
            first_batch = false;
        }
        else
        {
            ...
            auto flat_results = xt::flatten(results);
            auto flat_Y = xt::flatten(Y);
            results = xt::concatenate(xt::xtuple(flat_results, flat_Y));
    ...
    // cout << "Result: " << results << endl;
    // cout << "Argmax result: " << xt::argmax(results, -1) << endl;
    ...
    // if (make_decision) return xt::argmax(results, -1);
    // else return results;
}
